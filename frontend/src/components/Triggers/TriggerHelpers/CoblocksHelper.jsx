...
const CoblocksHelper = () => {
  ...
  const setTriggerData = () => {
    ...
    tmpNewFlow.triggered_entity_id = 'coblocks_form_submit'
  ...
  const handleFetch = () => {
    ...
    intervalRef.current = setInterval(() => {
      bitsFetch(null, 'coblocks/get').then(resp => {
        if (resp.success) {
          clearInterval(intervalRef.current)
          const tmpNewFlow = { ...newFlow }

          tmpNewFlow.triggerDetail.tmp = resp.data.coblocks
          tmpNewFlow.triggerDetail.data = resp.data.coblocks
          setNewFlow(tmpNewFlow)
          setIsLoading(false)
          setShowResponse(true)
          setSelectedFields([])
          bitsFetch({ reset: true }, 'coblocks/test/remove')
        }
      })
    }, 1500)
  }
  ...
  const info = `<h4>${__('Setup Coblocks', 'bit-integrations')}</h4>
            <a className="btcd-link" href="#" target="_blank" rel="noreferrer">${__(
  ...
  return (
    <div className="trigger-custom-width">
      ...
      <div className="flx flx-around">
        <button onClick={handleFetch} className="btn btcd-btn-lg sh-sm flx purple" type="button">
          {isLoading
      ...
      {newFlow.triggerDetail?.data && (
        <div className="flx flx-between">
          <button onClick={showResponseTable} className="btn btcd-btn-lg sh-sm flx">
            <span className="txt-coblocks-resbtn font-inter-500">
