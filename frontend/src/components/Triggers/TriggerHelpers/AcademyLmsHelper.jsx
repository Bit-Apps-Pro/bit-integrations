...
const AcademyLmsHelper = ({ flow, setFlowData }) => {
  const id = flow?.triggerData?.formID
  return (
    <div>
      {(id === '2' || id === '5') && (
        <>
          <h4>{__('Select a Quiz', 'bit-integrations')}</h4>
          <MultiSelect
            className="msl-wrp-options"
            defaultValue={flow.triggerData?.selectedQuiz}
            options={flow?.triggerData?.quizzes?.map(list => ({
      ...
      {id === '3' && (
        <>
          <h4>{__('Select a Lesson', 'bit-integrations')}</h4>
          <MultiSelect
            className="msl-wrp-options"
            defaultValue={flow.triggerData?.selectedLesson}
            options={flow?.triggerData?.lessons?.map(list => ({
      ...
      {(id === '4' || id === '1') && (
        <>
          ...
          <MultiSelect
            ...
            options={flow?.triggerData?.courses?.map(list => ({
      ...
      {id === '5' && flow.triggerData.selectedQuiz && (
        <>
          <h4>{__('Select a Condition', 'bit-integrations')}</h4>
          <MultiSelect
            className="msl-wrp-options"
            defaultValue={flow.triggerData?.selectedCondition}
            options={flow?.triggerData?.percentageCondition?.map(list => ({
              label: list.condition_title,
              value: list.condition_id.toString()
            }))}
            onChange={val => setFlowData(val, 'selectedCondition')}
            singleSelect
            style={{ width: '100%', minWidth: 300, maxWidth: 400 }}
          />
          <h4>{__('Target Value', 'bit-integrations')}</h4>
          <input
            className="btcd-paper-inp w-10 mt-1"
            onChange={e => setFlowData(e.target.value, 'requiredPercent')}
            name="requiredPercent"
            value={flow?.triggerData?.requiredPercent}
            type="text"
            placeholder={__('Targeted Percentage Value(ex: 50)', 'bit-integrations')}
          />
        </>
      )}
      <div />
...
export default AcademyLmsHelper
