...
export default function AsanaIntegLayout({
  formFields,
  handleInput,
  asanaConf,
  setAsanaConf,
  ...
  const handleActionInput = e => {
    ...
    newConf.field_map = [{ formField: '', asanaFormField: '' }]
    const { name } = e.target
    if (e.target.value !== '') {
      ...
      {asanaConf.actionName === 'task' && (
        <>
          ...
          <div className="flx">
            ...
            <MultiSelect
              options={asanaConf?.Projects?.map(Project => ({
                label: Project.name,
                value: Project.id
              }))}
      ...
      {asanaConf.actionName === 'task' && asanaConf.selectedProject && (
        <>
          ...
          <div className="flx">
            ...
            <MultiSelect
              options={asanaConf?.Sections?.map(Section => ({
                label: Section.name,
                value: Section.id
              }))}
              className="msl-wrp-options dropdown-custom-width"
      ...
      {asanaConf.actionName && (
        <div>
          ...
          <div className="flx flx-around mt-2 mb-2 btcbi-field-map-label">
            ...
            <div className="txt-dp">
              <b>{__('Asana Fields', 'bit-integrations')}</b>
            </div>
          </div>
