...
export const gravitecAuthentication = (
  ...
  if (!confTmp.site_url || !confTmp.app_key || !confTmp.app_secret) {
    setError({
      site_url: !confTmp.site_url ? __("Site Url can't be empty", 'bit-integrations') : '',
      app_key: !confTmp.app_key ? __("App Key can't be empty", 'bit-integrations') : '',
      app_secret: !confTmp.app_secret ? __("App Secret can't be empty", 'bit-integrations') : ''
    })
    return
  }
