...
export default function ZendeskIntegLayout({
  formFields,
  handleInput,
  zendeskConf,
  setZendeskConf,
  ...
  const handleActionInput = e => {
    ...
    newConf.field_map = [{ formField: '', zendeskFormField: '' }]
    const { name } = e.target
    if (e.target.value !== '') {
  ...
  const setChanges = (val, name) => {
    ...
    newConf[name] = val
    if (
      (name === 'selectedCRMCompany' || name === 'selectedCRMContact') &&
      val !== '' &&
      (zendeskConf.actionName === 'lead' || zendeskConf.actionName === 'deal')
    ) {
  ...
  return (
    <>
      ...
      {zendeskConf.actionName === 'lead' && (
        <>
          ...
          <div className="flx">
            ...
            <MultiSelect
              options={zendeskConf?.CRMCompanies?.map(CRMCompany => ({
                label: CRMCompany.name,
                value: CRMCompany.id
              }))}
      ...
      {zendeskConf.actionName === 'deal' && (
        <>
          ...
          <div className="flx">
            ...
            <MultiSelect
              options={zendeskConf?.CRMContacts?.map(CRMContact => ({
      ...
      {(zendeskConf.actionName === 'lead' || zendeskConf.actionName === 'deal') &&
        (zendeskConf.selectedCRMCompany || zendeskConf.selectedCRMContact) && (
          <>
            <br />
            <br />
            <div className="flx">
              ...
              <MultiSelect
                options={zendeskConf?.CRMSources?.map(CRMSource => ({
                  label: CRMSource.name,
                  value: CRMSource.id
                }))}
                className="msl-wrp-options dropdown-custom-width"
      ...
      {zendeskConf.actionName && (
        <div>
          ...
          <div className="flx flx-around mt-2 mb-2 btcbi-field-map-label">
            ...
            <div className="txt-dp">
              <b>{__('Zendesk Fields', 'bit-integrations')}</b>
            </div>
          </div>
