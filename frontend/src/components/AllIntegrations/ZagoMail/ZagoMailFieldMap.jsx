...
export default function ZagoMailFieldMap({ i, formFields, field, zagoMailConf, setZagoMailConf }) {
  ...
  const notResquiredField =
    zagoMailConf?.default?.fields &&
    Object.values(zagoMailConf?.default?.fields).filter(f => !f.required)
    ...
    <div className="flx mt-2 mb-2 btcbi-field-map">
      <div className="flx integ-fld-wrp">
        ...
        <select
          ...
          name="zagoMailField"
          value={field.zagoMailField}
          onChange={ev => handleFieldMapping(ev, i)}
          disabled={isRequired}>
          <option value="">{__('Select Field', 'bit-integrations')}</option>
          {isRequired
            ? zagoMailConf?.default?.fields &&
              Object.values(zagoMailConf.default.fields).map(fld => (
