...
export default function ConvertKitFieldMap({ i, formFields, field, convertKitConf, setConvertKitConf }) {
  ...
  const notResquiredField =
    convertKitConf?.default?.fields &&
    Object.values(convertKitConf?.default?.fields).filter(f => !f.required)
    ...
    <div className="flx mt-2 mb-2 btcbi-field-map">
      <div className="flx integ-fld-wrp">
        ...
        <select
          ...
          name="convertKitField"
          value={field.convertKitField}
          onChange={ev => handleFieldMapping(ev, i)}
          disabled={isRequired}>
          <option value="">{__('Select Field', 'bit-integrations')}</option>
          {isRequired
            ? convertKitConf?.default?.fields &&
              Object.values(convertKitConf.default.fields).map(fld => (
