    ...
    <div className="flx mt-2 mb-2 btcbi-field-map">
      <div className="pos-rel flx">
        <div className="flx integ-fld-wrp">
          <select
            ...
            <optgroup label={__('Form Fields', 'bit-integrations')}>
              {formFields?.map(f => (
                <option key={`ff-nx-${f.name}`} value={f.name}>
                  {f.label}
                </option>
              ))}
            </optgroup>
            ...
            <optgroup
              ...
              {isPro &&
                SmartTagField?.map(f => (
                  <option key={`ff-nx-smart-${f.name}`} value={f.name}>
                    {f.label}
                  </option>
                ))}
            </optgroup>
          ...
          {field.formField === 'custom' && (
            <TagifyInput
              onChange={e => handleCustomValue(e, i, notificationXConf, setNotificationXConf)}
        ...
        {i >= requiredFlds.length && (
          <>
            <button
              onClick={() => addFieldMap(i, notificationXConf, setNotificationXConf)}
              className="icn-btn sh-sm ml-2 mr-1"
              type="button">
              +
            </button>
            <button
              onClick={() => delFieldMap(i, notificationXConf, setNotificationXConf)}
