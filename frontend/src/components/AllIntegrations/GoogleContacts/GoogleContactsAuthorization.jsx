...
export default function GoogleContactsAuthorization({
  ...
  const { googleContacts } = tutorialLinks
  ...
  return (
    <div
      ...
      {googleContacts?.docLink && (
        <TutorialLink title="Google Contacts" docLink={googleContacts?.docLink} />
      )}
      ...
      <div className="mt-3">
        <b>{__('GoogleContacts Client Secret:', 'bit-integrations')}</b>
      </div>
      <input
        className="btcd-paper-inp w-6 mt-1"
        onChange={handleInput}
