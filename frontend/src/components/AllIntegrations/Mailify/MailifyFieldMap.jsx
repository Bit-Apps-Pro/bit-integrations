...
export default function MailifyFieldMap({ i, formFields, field, mailifyConf, setMailifyConf }) {
  ...
  const notResquiredField =
    mailifyConf?.default?.fields && Object.values(mailifyConf?.default?.fields).filter(f => !f.required)
    ...
    <div className="flx mt-2 mb-2 btcbi-field-map">
      <div className="flx integ-fld-wrp">
        ...
        <select
          ...
          name="mailifyField"
          value={field.mailifyField || ''}
          onChange={ev => handleFieldMapping(ev, i)}
          disabled={isRequired}>
          <option value="">{__('Select Field', 'bit-integrations')}</option>
          {isRequired
            ? mailifyConf?.default?.fields &&
              Object.values(mailifyConf.default.fields).map(fld => (
