...
export default function SendinBlueIntegLayout({
  ...
  const listOptions = () => {
    if (sendinBlueConf?.default?.sblueList) {
      return Object.values(sendinBlueConf.default.sblueList).map(sb => ({
        label: sb.name,
        value: String(sb.id)
      }))
    }
      ...
      {sendinBlueConf?.lists?.length !== 0 && (
        <>
          ...
          <div className="flx flx-around mt-2 mb-2 btcbi-field-map-label">
            ...
            <div className="txt-dp">
              <b>{__('Sendinblue Fields', 'bit-integrations')}</b>
            </div>
          </div>
          ...
          {sendinBlueConf.actions?.double_optin && (
            <>
              ...
              <small className="d-blk mt-5 ml-30">
                {__(
                  'To create and activate double optin email template , Please follow',
                  'bit-integrations'
                )}{' '}
                <a
                  className="btcd-link"
