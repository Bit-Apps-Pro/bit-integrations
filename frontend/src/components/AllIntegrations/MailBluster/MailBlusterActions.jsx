  ...
  return (
    <div className="pos-rel d-flx w-8">
      <TableCheckBox
        ...
        subTitle={__(
          'Note: If tag already exists, it will be only attached to the lead. Otherwise, it will be created first and then get attached.',
          'bit - integrations'
        )}
      />
      <TableCheckBox
        checked={mailBlusterConf.actions?.update || false}
        onChange={e => actionHandler(e, 'update')}
        className="wdt-200 mt-4 mr-2"
        value="update_contact"
        title={__('Update Lead', 'bit-integrations')}
        subTitle={__('Override the existing lead info by responses.', 'bit-integrations')}
      />
      <TableCheckBox
        checked={mailBlusterConf.actions?.doubleOptIn || false}
        onChange={e => actionHandler(e, 'doubleOptIn')}
        className="wdt-200 mt-4 mr-2"
        value="doubleOptIn"
        ...
        <div className="mt-2 ml-7 flx">
          ...
          <Cooltip Cooltip width={250} icnSize={17} className="ml-2 mr-18">
            <div className="txt-body">
              {__('Note: create custom options by pressing enter or comma', 'bit-integrations')}
              <br />
            </div>
          </Cooltip>
        </div>
