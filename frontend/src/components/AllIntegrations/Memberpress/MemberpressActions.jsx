...
export default function MemberpressActions({ memberpressConf, setMemberpressConf, formFields }) {
  ...
  const status = [
    { key: 'complete', label: __('Complete', 'bit-integrations') },
    { key: 'pending', label: __('Pending', 'bit-integrations') },
    { key: 'failed', label: __('Failed', 'bit-integrations') },
    { key: 'refunded', label: __('Refunded', 'bit-integrations') }
  ]
  ...
  return (
    <div className="pos-rel d-flx w-8">
      <TableCheckBox
        checked={memberpressConf?.actions?.status || false}
        onChange={e => actionHandler(e, 'status')}
        className="wdt-200 mt-4 mr-2"
        value="status"
        title={__('Add Status', 'bit-integrations')}
        subTitle={__('Add status of Membership', 'bit-integrations')}
      />
      ...
      <TableCheckBox
        checked={memberpressConf?.actions?.gateway || false}
        onChange={e => actionHandler(e, 'gateway')}
        className="wdt-200 mt-4 mr-2"
        value="gateway"
        ...
        {isLoading ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              ...
              options={
                memberpressConf?.default?.allPaymentMethods &&
                memberpressConf.default.allPaymentMethods.map(item => ({
                  label: item.paymentTitle,
            ...
            />
            <button
              onClick={() => paymentGateway(memberpressConf, setMemberpressConf, setIsLoading)}
