...
export const handleFieldMapping = (event, index, conftTmp, setConf, uploadFields, tab) => {
  ...
  if (tab) {
    if (uploadFields) {
      ...
      newConf.relatedlists[tab - 1].field_map[index][event.target.name] = event.target.value
    }
  } else if (uploadFields) {
...
export const delPhoneFieldMap = (i, confTmp, setConf) => {
  const newConf = { ...confTmp }
  if (newConf.phone_field && newConf.phone_field.length > 1) {
    newConf.phone_field.splice(i, 1)
  }
  setConf({ ...newConf })
}
