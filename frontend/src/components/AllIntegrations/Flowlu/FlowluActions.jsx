...
import {
  ...
  getAllManagers,
  getAllPortfolio,
  getAllProjectOpportunity,
  getAllProjectStage,
  getAllSource
} from './FlowluCommonFunc'
...
export default function FlowluActions({ flowluConf, setFlowluConf, loading, setLoading }) {
  ...
  return (
    <div className="pos-rel d-flx flx-wrp">
      {flowluConf.actionName === 'account' && (
        <TableCheckBox
          checked={flowluConf?.selectedCategory?.length || false}
          onChange={e => actionHandler(e, 'category')}
          className="wdt-200 mt-4 mr-2"
          value="category"
          title={__('Add Category', 'bit - integrations')}
          subTitle={__('Add Category')}
        />
      )}
      {flowluConf.actionName === 'account' && (
        <TableCheckBox
          checked={flowluConf?.selectedIndustry?.length || false}
          onChange={e => actionHandler(e, 'industry')}
          className="wdt-200 mt-4 mr-2"
          value="industry"
          title={__('Add Industry', 'bit - integrations')}
          subTitle={__('Add Industry')}
        />
      )}
      {flowluConf.actionName === 'opportunity' && (
        <TableCheckBox
          checked={flowluConf?.selectedSource?.length || false}
      ...
      {flowluConf.actionName === 'project' && (
        <TableCheckBox
          checked={flowluConf?.selectedPriority?.length || false}
          onChange={e => actionHandler(e, 'priority')}
          className="wdt-200 mt-4 mr-2"
          value="priority"
          title={__('Add Project Priority', 'bit - integrations')}
          subTitle={__('Add Project Priority')}
        />
      )}
      {(flowluConf.actionName === 'opportunity' || flowluConf.actionName === 'project') && (
        <TableCheckBox
          checked={flowluConf?.selectedCustomer?.length || false}
          onChange={e => actionHandler(e, 'customer')}
          className="wdt-200 mt-4 mr-2"
          value="customer"
          title={__('Add Customer', 'bit - integrations')}
          subTitle={__('Add Customer')}
        />
      )}
      {flowluConf.actionName === 'project' && (
        <TableCheckBox
          checked={flowluConf?.selectedProjectOpportunity?.length || false}
          onChange={e => actionHandler(e, 'projectOpportunity')}
          className="wdt-200 mt-4 mr-2"
          value="projectOpportunity"
          title={__('Add Project Opportunity', 'bit - integrations')}
          subTitle={__('Add Project Opportunity')}
        />
      )}
      {flowluConf.actionName === 'project' && (
        <TableCheckBox
          checked={flowluConf?.selectedManager?.length || false}
          onChange={e => actionHandler(e, 'manager')}
          className="wdt-200 mt-4 mr-2"
          value="manager"
          title={__('Add Project Manager', 'bit - integrations')}
          subTitle={__('Add Project Manager')}
        />
      )}
      {flowluConf.actionName === 'project' && (
        <TableCheckBox
          checked={flowluConf?.selectedProjectStage?.length || false}
          onChange={e => actionHandler(e, 'projectStage')}
          className="wdt-200 mt-4 mr-2"
          value="projectStage"
          title={__('Add Project Stage', 'bit - integrations')}
          subTitle={__('Add Project Stage')}
        />
      )}
      {flowluConf.actionName === 'project' && (
        <TableCheckBox
          checked={flowluConf?.selectedPortfolio?.length || false}
          onChange={e => actionHandler(e, 'portfolio')}
          className="wdt-200 mt-4 mr-2"
          value="portfolio"
          title={__('Add Project Portfolio', 'bit - integrations')}
          subTitle={__('Add Project Portfolio')}
        />
      )}
      ...
      <ConfirmModal
        ...
        {loading.accountCategories ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={flowluConf?.accountCategories?.map(category => ({
                label: category.name,
                value: `${category.id}`
              }))}
      ...
      <ConfirmModal
        ...
        {loading.industry ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={flowluConf?.industries?.map(industry => ({
                label: industry.name,
                value: `${industry.id}`
              }))}
              className="msl-wrp-options"
      ...
      <ConfirmModal
        ...
        {loading.source ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={flowluConf?.sources?.map(source => ({
                label: source.name,
                value: `${source.id}`
              }))}
              className="msl-wrp-options"
      ...
      <ConfirmModal
        ...
        {loading.customer ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={flowluConf?.customers?.map(customer => ({
      ...
      <ConfirmModal
        ...
        {loading.manager ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={flowluConf?.managers?.map(manager => ({
                label: manager.name,
                value: `${manager.id}`
              }))}
              className="msl-wrp-options"
      ...
      <ConfirmModal
        ...
        {loading.projectStage ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={flowluConf?.projectStages?.map(stage => ({
                label: stage.name,
                value: `${stage.id}`
              }))}
              className="msl-wrp-options"
      ...
      <ConfirmModal
        ...
        {loading.portfolio ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={flowluConf?.portfolios?.map(portfolio => ({
      ...
      <ConfirmModal
        ...
        {loading.projectOpportunity ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={flowluConf?.projectOpportunities?.map(Opportunity => ({
            ...
            />
            <button
              onClick={() => getAllProjectOpportunity(flowluConf, setFlowluConf, setLoading)}
              className="icn-btn sh-sm ml-2 mr-2 tooltip"
              style={{
