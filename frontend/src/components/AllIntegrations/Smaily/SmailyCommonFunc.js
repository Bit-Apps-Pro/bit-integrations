...
export const smailyAuthentication = (
  confTmp,
  setConf,
  setError,
  setIsAuthorized,
  ...
  if (!confTmp.name || !confTmp.subdomain || !confTmp.api_user_name || !confTmp.api_user_password) {
    setError({
      ...
      api_user_name: !confTmp.api_user_name
        ? __("Api user name can't be empty", 'bit-integrations')
        : '',
      api_user_password: !confTmp.api_user_password
        ? __("Api user password can't be empty", 'bit-integrations')
        : ''
    })
    return
  }
  ...
  bitsFetch(requestParams, 'smaily_authentication').then(result => {
