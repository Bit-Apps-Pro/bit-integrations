...
export default function ZohoRecruitActions({
  tab,
  formID,
  formFields,
  recruitConf,
    ...
    <>
      <div className="d-flx flx-wrp">
        {recruitConf?.relatedlists?.[tab - 1]?.module !== 'Notes' && (
          <>
            <TableCheckBox
              ...
              checked={
                tab === 0
                  ? 'workflow' in recruitConf.actions
                  : 'workflow' in recruitConf.relatedlists?.[tab - 1]?.actions
              }
              className="wdt-200 mt-4 mr-2"
              value="Workflow"
              title={__('Workflow', 'bit-integrations')}
              subTitle={__('Trigger workflows in Zoho Recruit.', 'bit-integrations')}
            />
            <TableCheckBox
              onChange={e => actionHandler(e, 'approval')}
              checked={
                tab === 0
                  ? 'approval' in recruitConf.actions
                  : 'approval' in recruitConf.relatedlists?.[tab - 1]?.actions
              }
              className="wdt-200 mt-4 mr-2"
              value="Approval"
              title={__('Approval', 'bit-integrations')}
              subTitle={__('Send entries to approval list in Zoho Recruit', 'bit-integrations')}
            />
        ...
        <TableCheckBox
          ...
          checked={
            tab === 0
              ? 'recordOwner' in recruitConf.actions
              : 'recordOwner' in recruitConf.relatedlists?.[tab - 1]?.actions
          }
          className="wdt-200 mt-4 mr-2"
          value="recordOwner"
          title={__('Record Owner', 'bit-integrations')}
          subTitle={__('Set owner of current record', 'bit-integrations')}
        />
      </div>
      <ConfirmModal
        ...
        <div className="flx flx-between">
          <input
            ...
            value={
              tab === 0
                ? recruitConf?.actions?.recordOwner || ''
                : recruitConf.relatedlists?.[tab - 1]?.actions?.recordOwner || ''
            }
            placeholder={__('Enter Owner ID', 'bit-integrations')}
          />
        </div>
      </ConfirmModal>
