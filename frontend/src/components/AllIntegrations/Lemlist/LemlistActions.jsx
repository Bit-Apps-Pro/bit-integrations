...
export default function LemlistActions({ lemlistConf, setLemlistConf }) {
  ...
  return (
    <div className="pos-rel d-flx w-8">
      <TableCheckBox
        checked={lemlistConf.actions?.update || false}
        onChange={e => actionHandler(e, 'update')}
        className="wdt-200 mt-4 mr-2"
        value="user_share"
        title={__('Update Lemlist', 'bit-integrations')}
        subTitle={__('Update Responses with Lemlist existing email?', 'bit-integrations')}
      />
    </div>
  )
}
