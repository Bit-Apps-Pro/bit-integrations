...
import {
  getAllCRMOwner,
  getAllCRMPipelines,
  getAllCRMPrimaryContact,
  refreshSalesmateFields
} from './SalesmateCommonFunc'
import SalesmateFieldMap from './SalesmateFieldMap'

export default function SalesmateIntegLayout({
  formFields,
  handleInput,
  salesmateConf,
  setSalesmateConf,
  ...
  return (
    <>
      ...
      <select
        ...
        <option value="4" data-action_name="deal">
          {__('Create Deal', 'bit-integrations')}
        </option>
        <option value="5" data-action_name="company">
          {__('Create Company', 'bit-integrations')}
        </option>
        <option value="6" data-action_name="product">
          {__('Create Product', 'bit-integrations')}
        </option>
      </select>
      ...
      {Number(salesmateConf.actionId) === 4 && !salesmateConf?.CRMContacts && (
        <>
          ...
          <br />
          <br />
          <small className="wdt-200 d-in-b error-msg">
      ...
      {Number(salesmateConf.actionId) === 4 && salesmateConf?.CRMContacts && (
        <>
          ...
          <div className="flx">
            ...
            <MultiSelect
              options={salesmateConf?.CRMContacts?.map(CRMContact => ({
                label: CRMContact.name,
                value: CRMContact.id.toString()
              }))}
            ...
            <button
              onClick={() =>
                getAllCRMPrimaryContact(salesmateConf, setSalesmateConf, loading, setLoading)
              }
      ...
      {((Number(salesmateConf.actionId) === 4 && salesmateConf?.selectedCRMContact) ||
        ...
        <>
          ...
          <div className="flx">
            ...
            <MultiSelect
              options={salesmateConf?.CRMOwners?.map(CRMOwner => ({
                label: CRMOwner.name,
      ...
      {Number(salesmateConf.actionId) === 4 && salesmateConf?.selectedCRMContact && (
        <>
          ...
          <div className="flx">
            ...
            <MultiSelect
              options={salesmateConf?.CRMPipelines?.map(CRMPipeline => ({
                label: CRMPipeline.pipeline,
                value: CRMPipeline.pipeline
              }))}
              className="msl-wrp-options dropdown-custom-width"
      ...
      {Number(salesmateConf.actionId) === 4 &&
        salesmateConf.selectedCRMPipeline &&
        salesmateConf?.selectedCRMContact && (
          <>
            <br />
            <div className="flx">
              <b className="wdt-200 d-in-b">{__('Select Stage:', 'bit-integrations')}</b>
              <MultiSelect
                options={salesmateConf?.stages.map(stage => ({
                  label: stage.stage,
                  value: stage.stage
                }))}
                className="msl-wrp-options dropdown-custom-width"
      ...
      {((Number(salesmateConf.actionId) === 4 && salesmateConf?.selectedCRMContact) ||
        (salesmateConf.actionName && Number(salesmateConf.actionId) !== 4)) && (
        <div>
          <br />
          <div className="mt-5">
            ...
            <button
              onClick={() =>
                refreshSalesmateFields(salesmateConf, setSalesmateConf, setIsLoading, setSnackbar)
              }
              className="icn-btn sh-sm ml-2 mr-2 tooltip"
          ...
          <div className="flx flx-around mt-2 mb-2 btcbi-field-map-label">
            ...
            <div className="txt-dp">
              <b>{__('Salesmate Fields', 'bit-integrations')}</b>
            </div>
          </div>
