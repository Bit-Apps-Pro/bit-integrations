...
import {
  getAllCRMCompany,
  getAllCRMCurrency,
  getAllCRMLostReasons,
  getAllCRMPriority,
  getAllCRMSources,
  getAllCRMStatus,
  getAllCRMTypes,
  getAllTags
} from './SalesmateCommonFunc'
...
export default function SalesmateActions({ salesmateConf, setSalesmateConf, loading, setLoading }) {
  ...
  return (
    <div className="pos-rel d-flx flx-wrp">
      {(Number(salesmateConf.actionId) === 1 ||
        Number(salesmateConf.actionId) === 4 ||
        Number(salesmateConf.actionId) === 5 ||
        Number(salesmateConf.actionId) === 6) && (
        <TableCheckBox
          checked={salesmateConf?.selectedTag?.length || false}
      ...
      {(Number(salesmateConf.actionId) === 1 || Number(salesmateConf.actionId) === 5) && (
        <TableCheckBox
          checked={salesmateConf?.selectedType?.length || false}
          onChange={e => actionHandler(e, 'type')}
          className="wdt-200 mt-4 mr-2"
          value="type"
          title={__('Add type', 'bit - integrations')}
          subTitle={__('Add a Type')}
        />
      )}
      {(Number(salesmateConf.actionId) === 1 ||
        Number(salesmateConf.actionId) === 4 ||
        ...
        Number(salesmateConf.actionId) === 6) && (
        <TableCheckBox
          checked={salesmateConf?.selectedCurrency?.length || false}
          onChange={e => actionHandler(e, 'currency')}
          className="wdt-200 mt-4 mr-2"
          value="currency"
          title={__('Add Currency', 'bit - integrations')}
          subTitle={__('Add a Currency')}
        />
      )}
      {Number(salesmateConf.actionId) === 4 && (
        <TableCheckBox
          checked={salesmateConf?.selectedLostReason?.length || false}
          onChange={e => actionHandler(e, 'lostReason')}
          className="wdt-200 mt-4 mr-2"
          value="lostReason"
          title={__('Add Lost Reasons', 'bit - integrations')}
          subTitle={__('Add a Lost Reason')}
        />
      )}
      {Number(salesmateConf.actionId) === 4 && (
        <TableCheckBox
          checked={salesmateConf?.selectedSource?.length || false}
          onChange={e => actionHandler(e, 'source')}
          className="wdt-200 mt-4 mr-2"
          value="source"
          title={__('Add Source', 'bit - integrations')}
          subTitle={__('Add a Source')}
        />
      )}
      {Number(salesmateConf.actionId) === 4 && (
        <TableCheckBox
          checked={salesmateConf?.selectedStatus?.length || false}
      ...
      {Number(salesmateConf.actionId) === 4 && (
        <TableCheckBox
          checked={salesmateConf?.selectedPriority?.length || false}
          onChange={e => actionHandler(e, 'priority')}
          className="wdt-200 mt-4 mr-2"
          value="priority"
          title={__('Add Priority', 'bit - integrations')}
          subTitle={__('Add Priority')}
        />
      )}
      {Number(salesmateConf.actionId) === 4 && (
        <TableCheckBox
          checked={salesmateConf?.selectedCompany?.length || false}
      ...
      {Number(salesmateConf.actionId) === 6 && (
        <TableCheckBox
          checked={salesmateConf?.selectedIsActive?.length || false}
          onChange={e => actionHandler(e, 'isActive')}
          className="wdt-200 mt-4 mr-2"
          value="isActive"
          title={__('Add Active for Sale', 'bit - integrations')}
          subTitle={__('Is Active or Not')}
        />
      )}
      ...
      <ConfirmModal
        ...
        {loading.CRMLostReason ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={salesmateConf?.lostReasons?.map(lostReason => ({
                label: lostReason,
                value: lostReason
              }))}
              className="msl-wrp-options"
      ...
      <ConfirmModal
        ...
        {loading.CRMPriority ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={salesmateConf?.priorities?.map(priority => ({
                label: priority,
                value: priority
              }))}
              className="msl-wrp-options"
      ...
      <ConfirmModal
        ...
        {loading.CRMCurrency ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={salesmateConf?.currencies?.map(currency => ({
      ...
      <ConfirmModal
        ...
        {loading.CRMCompany ? (
          ...
          <div className="flx flx-between mt-2">
            <MultiSelect
              options={salesmateConf?.companies?.map(company => ({
                label: company.name,
