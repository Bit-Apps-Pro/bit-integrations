...
export const refreshAutonamiListsAndTags = (
  autonamiConf,
  setAutonamiConf,
  setIsLoading,
  setSnackbar
  ...
  bitsFetch({}, 'autonami_lists_and_tags')
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({
          show: true,
          msg: __('Autonami lists and tags refreshed', 'bit-integrations')
        })
        ...
        setSnackbar({
          show: true,
          msg: `${__('Autonami lists and tags refresh failed Cause:', 'bit-integrations')}${
        ...
        setSnackbar({
          show: true,
          msg: __('Autonami lists and tags refresh failed. please try again', 'bit-integrations')
        })
      }
      setIsLoading(false)
    })
...
export const getAutonamiFields = (
  autonamiConf,
  setAutonamiConf,
  setIsLoading,
  setSnackbar,
  ...
  bitsFetch({}, 'autonami_fields')
    .then(result => {
      if (result && result.success) {
        ...
        if (result.data.autonamiFields) {
          ...
          if (!refreshFields) {
            ...
            newConf.field_map = Object.values(fields)
              .filter(f => f.required)
              .map(f => ({ formField: '', autonamiField: f.key, required: true }))
          }
        ...
        setSnackbar({
          show: true,
          msg: __('Autonami fields refresh failed. please try again', 'bit-integrations')
        })
      }
      setIsLoading(false)
    })
...
export const checkMappedFields = autonamiConf => {
  const mappedFields = autonamiConf?.field_map
    ? autonamiConf.field_map.filter(
        mappedField => !mappedField.formField && mappedField.autonamiField && mappedField.required
      )
    : []
