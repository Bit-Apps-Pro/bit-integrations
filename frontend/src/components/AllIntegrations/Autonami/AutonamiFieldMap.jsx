...
export default function AutonamiFieldMap({ i, formFields, field, autonamiConf, setAutonamiConf }) {
  ...
  const notResquiredField =
    autonamiConf?.default?.fields &&
    Object.values(autonamiConf?.default?.fields).filter(f => !f.required)
    ...
    <div className="flx mt-2 mb-2 btcbi-field-map">
      <div className="flx integ-fld-wrp">
        ...
        {field.formField === 'custom' && (
          <TagifyInput
            onChange={e => handleCustomValue(e, i, autonamiConf, setAutonamiConf)}
        ...
        <select
          ...
          name="autonamiField"
          value={field.autonamiField}
          onChange={ev => handleFieldMapping(ev, i)}
          disabled={isRequired}>
          <option value="">{__('Select Field', 'bit-integrations')}</option>
          {isRequired
            ? autonamiConf?.default?.fields &&
              Object.values(autonamiConf.default.fields).map(fld => (
