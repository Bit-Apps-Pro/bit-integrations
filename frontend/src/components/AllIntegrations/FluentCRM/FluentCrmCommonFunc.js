  ...
  bitsFetch({}, 'refresh_fluent_crm_lists')
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({
          show: true,
          msg: `${__('FluentCRM list refresh failed Cause:', 'bit-integrations')}${
  ...
  bitsFetch({}, 'refresh_fluent_crm_tags')
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({
          show: true,
          msg: `${__('FluentCRM tags refresh failed Cause:', 'bit-integrations')}${
            result.data.data || result.data
          }. ${__('please try again', 'bit-integrations')}`
        })
      } else {
  ...
  bitsFetch({}, 'fluent_crm_headers')
    .then(result => {
      if (result && result.success) {
        ...
        setSnackbar({
          show: true,
          msg: __('Fluent CRM fields refresh failed. please try again', 'bit-integrations')
        })
      }
      setIsLoading(false)
    })
...
export const getAllCompanies = (fluentCrmConf, setFluentCrmConf, loading, setLoading, setSnackbar) => {
  setLoading({ ...loading, company: true })
  bitsFetch({}, 'fluent_crm_get_all_company').then(result => {
...
const mapNewRequiredFields = fluentCrmConf => {
