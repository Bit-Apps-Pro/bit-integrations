...
export default function BitFormIntegLayout({
  formFields,
  handleInput,
  bitFormConf,
  setBitFormConf,
    ...
    <>
      ...
      <select onChange={handleInput} name="id" value={bitFormConf.id} className="btcd-paper-inp w-5">
        <option value="">{__('Select Form', 'bit-integrations')}</option>
        {
          ...
          bitFormConf?.default?.forms &&
            bitFormConf.default.forms.map(({ id, form_name }) => (
      ...
      <button
        onClick={() => fetchAllForm(bitFormConf, setBitFormConf, setIsLoading, setSnackbar)}
      ...
      {bitFormConf?.id && bitFormConf.default?.fields && (
        <>
          ...
          <div className="flx flx-around mt-2 mb-2 btcbi-field-map-label">
            ...
            <div className="txt-dp">
              <b>{__('Bit Form Fields', 'bit-integrations')}</b>
            </div>
          </div>

          {bitFormConf?.id &&
            bitFormConf?.field_map.map((itm, i) => (
              <BitFormFieldMap
                key={`sheet-m-${i + 9}`}
                i={i}
                field={itm}
