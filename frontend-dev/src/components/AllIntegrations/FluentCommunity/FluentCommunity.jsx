...
import { checkMappedFields, refreshCrmList } from './FluentCommunityCommonFunc'
...
export default function FluentCommunity({ formFields, setFlow, flow, allIntegURL }) {
  ...
  const nextPage = (val) => {
    ...
    if (val === 3) {
      ...
      setStep(val)
      // if (val === 2 && fluentCommunityConf.name) {
      //   refreshCrmList(formID, fluentCommunityConf, setFluentCommunityConf, setIsLoading, setSnackbar)
      // }
    }
  }
